...
%                                                                         %
%                                                                         %
% Run a Monte Carlo simulation of the general GPS system and compare      %
...
latency_th_cdf = latency_cdf_gps(xs, N, mu, nu, client);
paoi_th_cdf = paoi_cdf_gps(0.0001, 10, N, mu, nu, client);
T_th = transition_matrix_gps(N, mu, nu);
p_succ_th = success_prob_gps(N, client, T_th);

% Run Monte Carlo simulation and compute empirical CDFs
[paoi_mc, latency_mc, p_succ_mc, av_aoi_mc, load_mc] = montecarlo_gps(N, mu, nu, client, L);
...
av_aoi_mc
Y = expected_aoi_gps(N, mu, nu, client)
